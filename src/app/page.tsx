...
import { Search, Sparkles, Command, Database, Zap, Share2, MessageSquare, ChevronRight, Layers, LayoutDashboard, Globe, Download, Mail, ExternalLink, X, Shield, Lock, CheckCircle2, Cpu, BookOpen, FileText, ChevronDown, User, Bell, Settings, LogOut, Scale, Heart } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  {["ERP Portal (Academics)", "SNU Policy Handbook v2.0", "Student Welfare Guidelines"].map((src) => {
                    ...
                    return (
                      <div
                        key={src}
                        className={`px-3 py-2 rounded-xl border flex items-center gap-2 transition-all duration-300 ${isVisible
                          ? "border-indigo-500/20 bg-indigo-500/5 text-white/80"
                          : "border-white/5 bg-white/[0.01] text-white/20"
                          }`}
                      >
                        <div className={`w-4 h-4 rounded-full flex items-center justify-center border shrink-0 ${isVisible
          ...
          <div className="mx-auto max-w-5xl px-6 flex flex-col items-center">
            ...
            <div className="flex flex-col items-center gap-2 mt-4 text-center">
              <span className="text-[10px] uppercase tracking-[0.2em] text-white/20 font-bold">
                © {new Date().getFullYear()} SNUGPT ● Maintained by Rishabh Joshi
              </span>
